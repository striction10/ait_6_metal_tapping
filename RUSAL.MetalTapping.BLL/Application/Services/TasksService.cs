    ...
    IMapper mapper)
{
    ...
    {
        var entities = EnsureFound(await _tasksRepository.GetByShiftIdAsync(shiftId),
    ...
    {
        var entities = EnsureFound(await _tasksRepository.GetByBuildingAndDateRange(buildingId, from, to),
    ...
    {
        var entity = _mapper.Map<ShiftTask>(dto);

        await _tasksRepository.CreateAsync(entity);
    }
}